  ...
  get clientsStore(): OAuthRegisteredClientsStore {
    ...
    return {
      getClient(clientId) {
        ...
        // Legacy UUID client_ids cached by Cursor/Grok before attested DCR, wiped with ephemeral /data.
        // Safe on this single-user server: login still requires admin password; redirects must be allowlisted.
        if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(clientId)) {
          const stub = {
            ...
            ].filter((uri) => redirectAllowed(uri)),
            token_endpoint_auth_method: "none",
            grant_types: ["authorization_code", "refresh_token"],
            response_types: ["code"],
          } as OAuthClientInformationFull;
          if (!stub.redirect_uris.length) return undefined;
          try {
            store.update((d) => { d.oauth.clients[clientId] = stub as OAuthClient; });
          } catch { /* ignore */ }
          return stub;
        }
        return undefined;
